...
**Status:** paper complete and accepted at IJCAI-ECAI 2026 GlobalSouthAI (reviews 5/6/7); currently hardened and expanded for resubmission to a stronger venue. All core experiments (Tasks A–T) are done; a width-scaling study is in progress (see [Scaling work](#scaling-work-in-progress)). **Phase 2 (external validation, SAE mechanism, POMDP, theory) is underway** — asset verification, external behavioral validation (Gate 1, **passes 3/3 tasks**), the mandatory temporal-scrambling/EMA rigor controls, SAE mechanistic decomposition, a controlled full-vs-partial-observability (POMDP) stress test, positive causal steering, and a pre-scaling rigor-hardening pass are complete; see [outputs/deliverables/phase1_external_validation.md](outputs/deliverables/phase1_external_validation.md), [outputs/deliverables/phase2_rigor_controls.md](outputs/deliverables/phase2_rigor_controls.md), [outputs/deliverables/phase3_sae_decomposition.md](outputs/deliverables/phase3_sae_decomposition.md) (confusion direction is diffuse — Gate 2's honest outcome; a genuinely KL-non-redundant, externally-predictive SAE feature survives split-sample + cross-seed testing on **reacher only**, after the original 3-task claim was corrected), [outputs/deliverables/phase4_pomdp_stress_test.md](outputs/deliverables/phase4_pomdp_stress_test.md) (partial observability strengthens the signal's causal effect size and discriminability, with one honest, mechanistically-explained exception), [outputs/deliverables/phase5_filtering_theory.md](outputs/deliverables/phase5_filtering_theory.md) (a linear-Gaussian filtering simulation reproduces the qualitative pattern that `C_t`-like statistics are only informative under model misspecification/non-stationarity, never under a correctly-specified stationary filter), [outputs/deliverables/phase6_causal_steering.md](outputs/deliverables/phase6_causal_steering.md) (the confusion direction is decisively causal for its own probe readout against a proper empirical null, z=+7 to +16 on all 3 tasks; its causal effect on genuinely external imagination failure does **not** clear that null on any task, correcting an earlier overclaim), and [outputs/deliverables/pendulum_outlier_synthesis.md](outputs/deliverables/pendulum_outlier_synthesis.md) (the pendulum-outlier pattern holds on 3 independently-hardened legs, not 4 — one apparent causal confirmation was retracted after proper null-referencing).
