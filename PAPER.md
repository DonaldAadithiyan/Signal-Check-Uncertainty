...
**Scale.** All results are on a 256-dim GRU, 100K steps, 5-dim cartpole observations. The full DreamerV3 XL uses 4096-dim GRU, millions of steps, 64×64 image inputs. The scale question is open. The null-space geometry is a structural, content-based property of the representation (shown causally independent of z_gate in §6.2), arising from the RSSM objective's separation of observation-driven task content from slowly-accumulated confusion history rather than from a capacity constraint; we would expect it to persist at larger scale, but this is untested. The confusion integral time constant is not universal — best γ is identical (0.95) across 5 seeds on cartpole but is 0.70 on reacher and 0.90 on pendulum (see the Task diversity paragraph below), so it varies with environment dynamics.
