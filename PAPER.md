...
5. We operationalise the signal as an observation-routing policy that outperforms the recon-error baseline on two of three environments (cartpole and, strongly, pendulum, but not reacher — the payoff is environment-dependent; §4.5), and identify a second orthogonal signal — the obs/imagination boundary (1.0000 AUROC for the reported protocol, r = −0.015 to Probe A), which is largely a magnitude effect (§4.6) — demonstrating that h_t encodes at least two independent confusion-related aspects.
...
7. We characterise **generality across three structurally different environments** (cartpole, reacher, pendulum; reacher and pendulum replicated across 4 seeds; §6.1): the confusion direction, the null-space geometry, and the linear C_t encoding all generalise, while the closed-form γ/R² and the Set C contrastive metric are environment-specific. On pendulum the Set C metric *inverts below chance* — a seed-stable diagnostic-construction artefact we trace mechanistically to the sign of the within-KL-bin recon–C_t correlation, not an absence of signal (pendulum's C_t encoding is the strongest of the three). The core geometry and causal findings additionally hold at a 2× GRU width (one intermediate scale check; the full-scale question remains open).
...
**Set C (KL-matched) is the key result.** C1 and C2 have matched KL distributions (mean 22.9 vs 23.9 nats, 1-nat gap) but a 9× reconstruction gap (0.052 vs 0.471). Probe A at AUROC 0.72 is detecting something beyond KL magnitude — the accumulated confusion history in h_t. Figure `roc_curves` shows the dissociation on one set of axes: the confusion probe reads Set C (AUROC 0.71) while the ensemble baseline is near-uninformative there (0.59), and reconstruction error — a near-perfect novelty detector on direct OOD (1.00) — is a different signal entirely. (Figure `roc_curves_full_appendix` gives Probe A's full ROC on Sets A/B/C; Figure `cross_seed_stability` shows the per-seed Set C spread across all three environments.)
...
**Cross-environment routing: real but not universal.** We run the routing oracle on all three environments, with two router variants — Probe A (logistic on binary KL) and a C_t-direct router (Ridge on the continuous confusion integral, bypassing the Set C labelling) — against the recon-error oracle (recall@30% budget, top-25%-KL events, held-out N=40K, non-overlapping CIs throughout):

| environment | Probe-A recall | C_t-direct recall | recon-oracle recall | Probe-A gap |
|---|---|---|---|---|
| cartpole | 0.682 | 0.663 | 0.642 | +0.040 |
| reacher | 0.511 | 0.411 | **0.587** | **−0.077** |
| pendulum | **0.778** | 0.744 | 0.481 | **+0.297** |

The confusion router beats the recon oracle on cartpole and — most strongly — on pendulum, but *loses* on reacher, where reconstruction error is itself an unusually strong routing signal. The routing payoff is therefore real but **environment-dependent**, not universal. Two further observations. First, on **pendulum the router is robust (+0.30) despite the Set C diagnostic inverting there (0.32)** — routing scores against KL events directly and is immune to the recon-based labelling artefact that inverts Set C (§6.1), the clearest evidence that the inversion is an evaluation-construction property, not a signal-quality one. Second, the two router variants move together in every environment (both win on cartpole/pendulum, both lose on reacher) and C_t-direct is consistently *slightly* weaker than Probe A — so, contrary to what the Set C inversion might suggest, routing gains nothing from bypassing the KL-trained probe.

### 4.6 Three orthogonal signals in h_t

**The observation/imagination boundary.** A probe trained to separate real posterior h_t (label 0) from imagined h_t (label 1) across all imagination depths 1–15 achieves **AUROC 1.0000** on the held-out-training-states evaluation (40K real + 75K imagined). This separability is protocol-dependent in a sensible direction: it grows with imagination depth (a 5-depth imagined pool scores 0.88–1.0 depending on the real-state source, versus ≈1.0 for the full 1–15 pool), because deeper imagination drifts further off the posterior manifold. We therefore report the boundary AUROC with its evaluation protocol rather than as a universal constant; at the reported protocol (depths 1–15, held-out training reals) it is 1.0000. After one imagination step with random actions, h_t is essentially separable from real posterior states by a linear classifier.
...
**Scale.** All main results are on a 256-dim GRU, 100K steps, 5-dim cartpole observations. The full DreamerV3 XL uses 4096-dim GRU, millions of steps, 64×64 image inputs. The scale question is not settled, but it is no longer entirely untested: at a 2× GRU width (deter=512), the two most load-bearing findings both survive — the confusion direction remains near-orthogonal to the top PCs (89.4°, 0.13% of its variance in the top-10 PCs, vs ~88°/0.5% at XS), and ablating it still collapses the probe readout (−0.29 at the intervention step vs −0.05 for a random-direction control). This is one intermediate data point, not a resolution: it is a single seed with a light single-control ablation, still three orders of magnitude below the full model. The null-space geometry is a structural, content-based property of the representation (shown causally independent of z_gate in §6.2), arising from the RSSM objective's separation of observation-driven task content from slowly-accumulated confusion history rather than from a capacity constraint; it holds at one intermediate scale beyond XS, and we would expect it to persist further, but the full-scale question remains open. The confusion integral time constant is not universal — best γ is identical (0.95) across 5 seeds on cartpole but is 0.70 on reacher and 0.90 on pendulum (see the Task diversity paragraph below), so it varies with environment dynamics.

**Task diversity (three environments).** We evaluate on three structurally different dm_control environments: cartpole-swingup (5-dim obs, 1-dim action), reacher-easy (6-dim, 2-dim, a 2-link arm), and pendulum-swingup (3-dim, 1-dim, a single underactuated link). What generalizes across all three: (i) the **null-space geometry** — the confusion direction is 88.0°/89.4°/88.1° from the top PCs with ≤9% of its variance there in every case; and (ii) the **linear encoding of the confusion integral C_t** — R² = 0.80/0.22/0.89, above the KL baseline in each environment (pendulum is in fact the strongest). What does *not* generalize: (iii) the **closed-form parameters** — best γ = 0.95/0.70/0.90 and C_t R² span 0.22–0.89 are environment-dependent; and (iv) the **Set C contrastive AUROC** — 0.72/0.62/0.32, which *inverts* on pendulum. Crucially, pendulum has the strongest C_t encoding (R² = 0.89) yet an inverted Set C (0.32, 95% CI [0.27, 0.38]), dissociating the two: the Set C inversion is an artefact of the recon-based C1/C2 labelling anti-aligning with confusion history in pendulum's dynamics, not an absence of signal (Probe C on Set C and the within-task confound invert with it). We therefore claim generality for the confusion direction, the null-space geometry, and the linear C_t encoding, while scoping the specific γ/R² and the Set C metric to cartpole. This is a three-point pattern, not an n=2 comparison: neither reacher nor cartpole is the outlier — the structure generalizes, the specifics do not. We replicate reacher and pendulum across 4 seeds each: **pendulum's Set C inversion is stable** (all 4 seeds below 0.5, 0.396 ± 0.063), so the mechanism above rests on replicated evidence rather than a single run, and pendulum's C_t R² is stably the strongest (0.855 ± 0.038); best γ (0.70, 0.90) and the null-space angle are seed-invariant. One honest caveat surfaced by replication: **reacher's within-task confound is seed-variable and centres above chance** (0.654 ± 0.084 across seeds, versus 0.578 for the single run) — reacher's confound control is the least clean of the three environments, and the single-seed number understated its variability. Genuinely different domains (locomotion, manipulation, image inputs) remain untested.

**Why Set C inverts on pendulum — a tested mechanism.** Set C splits its contrastive groups (C1 low recon, C2 high recon) *within KL bins*, assuming per-step reconstruction error proxies confusion. Whether this holds depends on the **within-KL-bin correlation between recon error and the accumulated confusion C_t**, which we measure directly: it is +0.39 on cartpole, −0.09 on reacher, and −0.12 on pendulum — monotone with the Set C AUROC (0.72 / 0.62 / 0.32; Figure `setc_inversion_mechanism`). On cartpole, high-recon states within a KL bin are also high-confusion, so the recon label agrees with what the probe reads; on pendulum the two anti-correlate, so the label runs opposite to confusion and the AUROC inverts below chance. Two controls sharpen this: (i) the alternative hypothesis that pendulum's recon is dominated by dynamically hard instantaneous kinematics (near-upright, high angular velocity) is *ruled out* — recon correlates more with C_t than with any kinematic feature on pendulum, and it is cartpole where kinematics dominate recon yet Set C works; (ii) at matched KL, KL already explains most of C_t on pendulum (r(KL,C_t)=0.67), so relabelling Set C by C_t directly only recovers it to chance (0.53), confirming Set C is an inherently low-signal test there whose *sign* is set by the recon↔C_t alignment. The practical takeaway, transferable to any new environment: the within-KL-bin sign of recon↔C_t predicts whether a recon-based contrastive construction will read confusion, be uninformative, or invert.

**Reacher is the least-clean environment — one coherent limitation.** Three observations about reacher, from three different tasks, describe a single underlying fact and should be read together rather than as separate hedges. (i) Its within-task confound control is the least clean of the three: near chance on the original single seed (0.578) but seed-variable and centring *above* chance across four seeds (0.654 ± 0.084, range [0.565, 0.751]). (ii) Its closed-form C_t characterisation is the weakest (R² = 0.26, vs 0.80/0.86 on cartpole/pendulum) — the linear C_t signal, while present, is faintest here. (iii) It is the one environment where the confusion router *loses* to the reconstruction-error oracle (recall 0.51 vs 0.59 at a 30% budget). The common thread is that on reacher's dynamics, per-step reconstruction error is itself an unusually strong and confusion-aligned signal, which simultaneously makes recon a hard routing baseline to beat, leaves less distinct confusion residual for the probe to read at matched KL, and lets task-structure leak into the within-task control. Reacher does not contradict the core findings — its Set C stays above chance on all four seeds and its null-space geometry and best-γ are rock-stable — but it is the environment where the confusion signal is least distinct from reconstruction error, and the paper scopes its claims accordingly.

**Probe-weighted returns: a null result.** Using probe scores to weight imagined returns (Task 2) degraded return quality (Δr = −0.53). The cause: imagined prior entropy is 14× smaller than real KL at this training level — the imagination is not calibrated to real dynamics. This mechanism would require a calibrated world model; at 100K steps it does not hold. A structurally different mechanism — using confusion as an imagination *stopping rule* that truncates the rollout on high confusion, rather than a reweighting term — also fails to improve value-estimate correlation (cross-sample Δr = +0.005 ± 0.009, sign-unstable across resampled start populations), though unlike reweighting it does not degrade it. Across two structurally different mechanisms, confusion is diagnostic, not value-shaping, in this setup.
...
**World model uncertainty:** RSSM-based uncertainty has been studied primarily via ensemble disagreement [Chua et al., 2018; Kidambi et al., 2020], predictive variance [Janner et al., 2019], and distributional shift detection [Lütjens et al., 2019]. Recent Dreamer-family work uses ensemble-disagreement epistemic uncertainty to steer imagined exploration [Probabilistic Dreaming; Wong, 2026]. All of these measure *between-model* novelty. Our work measures confusion — a *within-model*, within-distribution property that these methods cannot detect (§4.1).
...
**Causal probing and the interpretability illusion:** Makelov et al. (2024) show subspace activation patching can produce behaviour consistent with a feature via a dormant parallel pathway unrelated to the model's normal computation, and Sklar (2023) catalogues concrete mitigations. We adopt those mitigations — an empirical null distribution over many random directions rather than a single control, a mechanistically distinct downstream measure, a real-content-substitution (rather than synthetic) intervention, and perturbation-robustness and cross-seed replication checks — to establish that the confusion direction's causal effect is not such an artefact (§4.7).

**Representation probing:** Linear probes have been used extensively in NLP [Conneau et al., 2018; Tenney et al., 2019] and vision [Alain & Bengio, 2016]. Probing for uncertainty in neural representations has been studied in supervised settings [Gal & Ghahramani, 2016; Lakshminarayanan et al., 2017]. Closest to our setting, prior work probes emergent *task-state* representations inside learned world models — e.g., linear board-state directions in an Othello-playing sequence model [Nanda et al., 2023] and emergent planning computation in a recurrent Sokoban agent [Taufeeque et al., 2024]. Those works recover what the model represents about the task; we instead probe a *meta* property — the model's accumulated confusion about its own predictions — and show, with a contrastive evaluation that controls for KL magnitude, that it lives in the representation's near-null space. To our knowledge this is the first work to apply probing to uncertainty in world model hidden states in this way.
...
We have shown that a DreamerV3 world model's recurrent state h_t implicitly encodes a within-task confusion signal that is distinct from novelty, detectable at AUROC 0.72 (5-seed mean 0.715 ± 0.074, 95% CI [0.666, 0.763]) on KL-matched contrastive sets, and approximately equal to a discounted confusion count C_t (R² = 0.80, γ = 0.95 across all 5 seeds). The signal lives in the near-null space of h_t's PCA (88° from all top PCs, 9% in high-variance subspace) — a structural, content-based property of the representation that we show causally is independent of GRU update-gate saturation (forcing the gate from 0.5 to 0.99 at inference leaves the geometry unchanged), superseding an earlier gate-saturation account that was falsified. We further show, by activation ablation against a 50-direction empirical null, that the confusion direction is causally load-bearing — removing it collapses the probe readout at the 100th percentile of the null, an effect that replicates across all 5 seeds. Taking Δh_t recovers cross-task generalisation (0.70 AUROC) without multi-task training; across three structurally different environments (cartpole, reacher, pendulum — the latter two replicated across 4 seeds each) the confusion direction, the null-space geometry, and the linear C_t encoding all generalise, while the closed-form time constant and the Set C contrastive metric are environment-specific. On pendulum the Set C metric even inverts (below chance, stable across all 4 seeds) — a diagnostic-construction artefact we trace mechanistically to the sign of the within-KL-bin recon–C_t correlation, not an absence of signal (pendulum's underlying C_t encoding is in fact the strongest of the three). These core findings also survive at a 2× GRU width (an intermediate scale check), though the full-scale question remains open. The signal operationalises as an observation-routing oracle that outperforms the recon-error baseline on two of the three environments (cartpole, and strongly on pendulum despite its Set C metric inverting), but not on reacher — the routing payoff is real but environment-dependent. The signal is also reinforcing with the latent attractor-recovery / reward-overestimation phenomenon of Berger et al. — flagging the same problematic states at inference time at no cost. Finally, a second orthogonal signal — the observation/imagination boundary, at AUROC 1.0000 for the reported evaluation protocol (a separability that grows with imagination depth), largely a magnitude effect — coexists in the same near-null space with r = −0.015 correlation to the confusion probe.
...
Berger, J., Frauenknecht, B., Trimpe, S., & Leibe, B. (2026). Biased Dreams: Limitations to Epistemic Uncertainty Quantification in Latent Space Models. arXiv:2604.25416.
...
Lütjens, B., Everett, M., & How, J. P. (2019). Safe Reinforcement Learning with Model Uncertainty Estimates. ICRA 2019.

Makelov, A., Lange, G., Geiger, A., & Nanda, N. (2024). Is This the Subspace You Are Looking For? An Interpretability Illusion for Subspace Activation Patching. ICLR 2024. arXiv:2311.17030.

Nanda, N., Lee, A., & Wattenberg, M. (2023). Emergent Linear Representations in World Models of Self-Supervised Sequence Models. BlackboxNLP Workshop, EMNLP 2023. arXiv:2309.00941.

Pathak, D., Agrawal, P., Efros, A. A., & Darrell, T. (2017). Curiosity-driven exploration by self-supervised prediction. ICML 2017.

Schmidhuber, J. (1991). A possibility for implementing curiosity and boredom in model-building neural controllers. SAB 1991.

Sklar, M. (2023). 6 Ways to Fight the Interpretability Illusion. Confirm Labs (technical post), Nov. 30, 2023. https://confirmlabs.org/posts/fight_the_illusion.html

Taufeeque, M., Quirke, P., Li, M., Cundy, C., Tucker, A. D., Gleave, A., & Garriga-Alonso, A. (2024). Planning in a recurrent neural network that plays Sokoban. Mechanistic Interpretability Workshop, ICML 2024. arXiv:2407.15421.

Tenney, I., Das, D., & Pavlick, E. (2019). BERT Rediscovers the Classical NLP Pipeline. ACL 2019.

Wong, G. (2026). Probabilistic Dreaming for World Models. arXiv:2603.04715.
